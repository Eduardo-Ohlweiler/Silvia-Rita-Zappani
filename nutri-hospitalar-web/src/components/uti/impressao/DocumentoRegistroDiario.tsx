...
export function DocumentoRegistroDiario({ registro }: { registro: RegistroDiarioUtiResponse }) {
   ...
   *
   * Imprimir o digitado ao lado de um percentual calculado sobre outro número
   * dá uma conta que não fecha num prontuário — 1.200 de 1.500 não é 88,0 %, e
   * quem confere no papel conclui que o sistema errou. Hoje quem resolve a
   * precedência é o servidor, e manda o número escolhido junto da procedência:
   * a folha só o mostra, e não há cópia da regra aqui.
   */
  const dieta: LinhaValor[] = [
    { rotulo: 'Dieta', valor: r.dieta ?? '—' },
    {
      rotulo: 'Volume prescrito em 24 h',
      valor: n(r.prescritoDeReferencia, 0, 'ml'),
